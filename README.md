...
- **Attention ≠ DEG significance:** Top attention genes (by temporal CV) show zero overlap with Liao et al.'s top DEGs at 1h and 72h in L2/3 IT neurons (hypergeometric p=1.0, 0/30 overlap at both timepoints, below chance expectation).
...
delta-expression-encoder/
├── README.md
├── deg_encoder_prototype.py      # Training pipeline — data loading, model, training loop
├── deg_encoder_investigate.py    # Analysis script — all paper figures and results
└── notebooks/
    ├── deg_encoder_prototype.ipynb   # Training notebook with embedded outputs
    └── deg_encoder_investigate.ipynb # Analysis notebook with embedded outputs
```
...
# Full model class definition in deg_encoder_prototype.py
...
The full training pipeline is in `deg_encoder_prototype.py`. It requires the processed AnnData from Zenodo and auto-resumes from checkpoints. Run top-to-bottom in a Colab session with GPU.
...
Checkpoints include `delta_v5_v2_epoch50.pt` (best embedding structure, used for representation analyses) and `delta_v5_v2_final.pt` (epoch 300, 69.4% weighted accuracy, used for classification results).

---

## Reproducing paper figures

All figures are generated by `deg_encoder_investigate.py`. After loading the model and artifacts from Zenodo:
...
| Fig 3B | Block 6 | Accuracy vs DEG magnitude scatter |
...
| Index | Cell type string | Class |
|---|---|---|
| 0 | `004 L6 IT CTX Glut` | Excitatory |
| 1 | `005 L5 IT CTX Glut` | Excitatory |
| 2 | `006 L4/5 IT CTX Glut` | Excitatory |
| 3 | `007 L2/3 IT CTX Glut` | Excitatory — primary psilocybin target |
| 4 | `022 L5 ET CTX Glut` | Excitatory |
| 5 | `029 L6b CTX Glut` | Excitatory |
| 6 | `030 L6 CT CTX Glut` | Excitatory |
| 7 | `032 L5 NP CTX Glut` | Excitatory |
| 8 | `046 Vip Gaba` | Inhibitory |
| 9 | `047 Sncg Gaba` | Inhibitory |
| 10 | `049 Lamp5 Gaba` | Inhibitory |
| 11 | `052 Pvalb Gaba` | Inhibitory |
| 12 | `053 Sst Gaba` | Inhibitory |
| 13 | `319 Astro-TE NN` | Non-neuronal |
| 14 | `326 OPC NN` | Non-neuronal |
| 15 | `327 Oligo NN` | Non-neuronal |
| 16 | `333 Endo NN` | Non-neuronal |
| 17 | `334 Microglia NN` | Non-neuronal |
